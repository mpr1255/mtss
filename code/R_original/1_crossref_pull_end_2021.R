...
# NOTE!! The space between the = and the email is necessary! The quotation marks are necessary. 
# See https://github.com/ropensci/rcrossref#register-for-the-polite-pool and https://stackoverflow.com/questions/68412035/rcrossref-api-error-resresponse-headerscontent-type-application-jsonch and more. It's necessary to get this EXACTLY right or you will get weird error messages about utf8 and other things.  

here <- here()

polisci_journals <- fread(glue("{here}/output/JCR_Political_Science.csv"))
ir_journals <- fread(glue("{here}/output/JCR_International_Relations.csv"),header = TRUE)
...
for(i in seq_len(nrow(journals))){
  # i <- 1
  outname <- glue("{here}/data/crossref_dumps/{journals$journal_name[i]}_end_2021.Rds")
  ...
  if(file.exists(outname)){
    ...
    try(res[i] <- tibble(V1 = cr_journals(issn = journals$issn[i], cursor = "*", cursor_max = 1, works = TRUE, filter=c(from_pub_date='2021-01-01', until_pub_date='2022-01-01'))))

...
An unfortunate oversight on our part. We have now fixed this.
...
We thank the reviewer for this perspective and highlight that we take a descriptive view in our analysis of preregistration rates, alongside summarising what has been written in the open science literature about why preregistration is a desirable practice. By taking a descriptive view we focus on identifying experiments and determine whether we can find an accompanying preregistration document. In doing so, we hope to stimulate discussion about what the desired percentage might be. This point has been added to the discussion section. Determining post hoc which experiments should and should not be preregistered, as well as specifying the correct number of experiments that should comprise the preregistration denominator, is a conceptually enormously difficult task and one we believe falls outside our remit. 
...
We report journals and have added more figures illustrating journal open data and preregistration rates in the appendix. 
...
We were slightly uncertain about the meaning of this comment but we take it as broader commentary about how statistical inference is practiced in the discipline. 
...
We thank the reviewer for pointing out the omission of confusion matrices in the appendix. We have added the SVM confusion matrices for the statistical inference and experiment papers, which is the classifier we used to perform our main analysis. The figure also includes the main statistics during the train-test process. These statistics indicate the model performs very well and particularly in identifying statistical papers with 92.35% accuracy and 94% sensitivity. Please note that we report the results from the `./output/ml_svm_stat_test.txt` and `./output/ml_svm_exp_test.txt` validation files, which were generated in 2021. These results trivially differ from the confusion matrices reported in the appendix because we reran the models for this revise and resubmit and a different train-test split was used.^[The confusion matrices were generated with the `./code/R_original/test_ML/test_experiments.R` and `./code/R_original/test_ML/test_stat_inf.R` scripts). ]
